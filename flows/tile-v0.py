...
def configure_pipeline(
    dest, indicator_bucket, model_bucket, model_id, run_id, compute_tiles, is_indicator
) -> Tuple[dict, str, str, bool, bool, bool, bool, bool]:
    if is_indicator:
...
def temporal_aggregation(df, time_res, should_run):
    ...
    t = dd.to_datetime(df["timestamp"], unit="s").apply(
...
def compute_timeseries(df, dest, time_res, model_id, run_id):
    ...
    timeseries_df = timeseries_df.groupby(["feature"]).apply(
        lambda x: save_timeseries(
            x, dest, model_id, run_id, time_res, timeseries_agg_columns
        ),
        meta=(None, "object"),
    )
...
def subtile_aggregation(df, should_run):
    ...
    subtile_df = (
        subtile_df.rename(columns=spatial_lookup)
        .drop(columns="s_count_t_sum")
        .reset_index()
    )
    return subtile_df
...
def compute_tiling(df, dest, time_res, model_id, run_id):
    ...
    tiling_df = (
        ...
        .apply(
            lambda x: save_tile(
                to_proto(x), dest, model_id, run_id, x.feature, time_res, x.timestamp
            ),
            axis=1,
            meta=(None, "object"),
        )
    ...
    for level in range(len(regions_cols)):
        ...
        save_df = save_df.apply(
            lambda x: save_regional_aggregation(
                x, dest, model_id, run_id, time_res, region_level=regions_cols[level]
            ),
            axis=1,
            meta=(None, "object"),
        )
...
def save_raw_data(df, dest, time_res, model_id, run_id, should_run):
    ...
    raw_df = df.groupby(["feature"]).apply(
        lambda x: raw_data_to_json(
            x[output_columns], dest, model_id, run_id, time_res, x["feature"].values[0]
        ),
        meta=(None, "object"),
    )
    raw_df.compute()
...
def compute_output_summary(df):
    ...
    timeseries_df = df.groupby(["feature", "timestamp"]).agg(
        {"t_mean": timeseries_aggs}
    )
    timeseries_df.columns = timeseries_df.columns.to_flat_index()
    timeseries_df = timeseries_df.rename(columns=timeseries_lookup).reset_index()

    summary = output_values_to_json_array(
        timeseries_df[["feature", timeseries_agg_column]]
    )
    return summary
...
with Flow("datacube-ingest-v0.1") as flow:
    ...
    monthly_spatial_data = subtile_aggregation(
        monthly_data, compute_tiles, upstream_tasks=[month_ts_done]
    )
    month_stats_done = compute_stats(
        monthly_spatial_data, dest, "month", model_id, run_id
    )
    month_done = compute_tiling(
        monthly_spatial_data,
        dest,
        "month",
        model_id,
        run_id,
        upstream_tasks=[month_stats_done],
    )

    # ==== Run aggregations based on annual time resolution =====
    annual_data = temporal_aggregation(
        df, "year", compute_annual, upstream_tasks=[month_done]
    )
    ...
    annual_spatial_data = subtile_aggregation(
        annual_data, compute_tiles, upstream_tasks=[year_ts_done]
    )
...
if __name__ == "__main__":
    with raise_on_exception():
        executor = DaskExecutor(
            # address="tcp://10.65.18.58:8786"
        )  # Dask Dashboard: http://10.65.18.58:8787/status
        state = flow.run(
            executor=executor,
            parameters=dict(
                is_indicator=True,
                model_id="ACLED",
                run_id="indicator",
                data_paths=["s3://test/acled/acled-test.bin"],
            ),
        )
    ...
    # state = flow.run(executor=executor, parameters=dict(compute_tiles=True, model_id='maxhop-v0.2', run_id='4675d89d-904c-466f-a588-354c047ecf72',
