...
ELASTIC_URL = 'http://10.65.18.34:9200'
...
qualifiers_to_ignore = ['timestamp', 'lat', 'lng', 'feature', 'value', 'country', 'admin1', 'admin2', 'admin3', 'admin4']

headers={'Accept': 'application/json', 'Content-type': 'application/json'}

def get_qualifier_values(data_id, feature_name, qualifier_name, timestamp):
    url = 'http://localhost:8080/api/maas/output/qualifier-data?data_id=' + data_id + '&run_id=indicator&feature=' + feature_name + '&resolution=month&temporal_agg=mean&spatial_agg=mean&timestamp=' + str(timestamp) + '&qlf[]=' + qualifier_name
    ...
    if response:
        ...
        if results[0] and results[0]['options']:
            ...
            for value in options:
                options_values.append(value['name'])
            return options_values
    return []

def getDefaultTimestamp(data_id, feature_name):
    url = 'http://localhost:8080/api/maas/output/timeseries?data_id=' + data_id + '&run_id=indicator&feature=' + feature_name + '&resolution=month&temporal_agg=mean&spatial_agg=mean'
    response = requests.get(url)
    #print(url)
    results = response.json()
...
def update_metadata(doc_id, indx):
    url = 'http://10.65.18.69:9200/data-datacube/_update_by_query'
    print(doc_id)
    query = json.dumps({
        "script" : {
            "source": "ctx._source.qualifier_outputs[{}].is_visible = true".format(indx),
            "lang": "painless"
        },
        "query": {
            "bool": {
...
for doc_id in TEST:
    ...
    response = requests.get('http://10.65.18.34:9200/data-datacube/_search', data=query, headers=headers)
    results = response.json()
    ...
    for ind in indicator_list:
        ...
        if qualifier_outputs:
            ...
            valid_qualifier_outputs = list(filter(lambda x: (not x['name'] in qualifiers_to_ignore) , qualifier_outputs))
            if len(valid_qualifier_outputs) > 0:
                ...
                for qualifer_output in valid_qualifier_outputs:
                    ...
                    qualifier_index = -1
                    for index, item in enumerate(qualifier_outputs):
                        if item['name'] == qualifier_name:
                            qualifier_index = index
                            break
                    qualifier_values = get_qualifier_values(doc_id, feature_name, qualifier_name, timestamp)
                    ...
                    if qualifier_values_count <= 20:
                        ...
                        print('updating: ' + qualifier_id + ' for qualifier: ' + qualifier_name + ' indexed at ' + str(qualifier_index))
